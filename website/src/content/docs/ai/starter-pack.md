---
title: Cardano Client Bindings — AI Starter Pack
description: Everything an AI agent needs to write correct code against the cardano-client-lib bindings on first try. Ingest this before generating code.
---
...
with CclLib() as lib:                                # context manager; or lib.close()
    ...
    # or fully offline: lib.quicktx.build(yaml, utxos, protocol_params, exec_units=None)
...
JS: `bridge.quicktx.build(yaml, utxos, params)` / `bridge.account.signTx(mnemonic, TESTNET, 0, 0, txCbor)`.
Rust: `bridge.quicktx().build(&yaml, &utxos, &params)?` / `bridge.account().sign_tx(&mnemonic, TESTNET, 0, 0, &tx_cbor)?`.
...
- **quicktx**: `build(yaml, utxos, protocol_params, exec_units=None)`, `build_with(yaml, provider, sender, evaluator=None)` → `{tx_cbor, tx_hash, fee}` (unsigned).
