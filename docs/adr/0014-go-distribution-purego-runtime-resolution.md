...
2. **Resolve the library at runtime** — Go's only hook. In order:
   `CCL_LIB_PATH` → a per-version user cache (`os.UserCacheDir()`) → a **one-time download** of the
   ...
   `build.rs` fetch — same version, tarball naming, and `CCL_LIB_VERSION` override — only shifted from
...
- First use needs the network **once** (then cached), unless `CCL_LIB_PATH` is set — the trade-off for
...
- **Keep cgo (+ build-time `-lccl`).** Undistributable — needs the library at link time with no hook
