...
// native build/release matrix; used to locate the `@bloxbean/ccl-<suffix>` optionalDependency.
...
//   4. the `@bloxbean/ccl-<platform>` optionalDependency package — the published layout;
...
export function resolveLibFile(libPath) {
  ...
  try {
    const fromPkg = Bun.resolveSync(`@bloxbean/ccl-${platformSuffix()}/${name}`, import.meta.dir);
