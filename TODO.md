...
- [~] `P1` **Investigate static linking** — *spiked; see `docs/spikes/static-linking.md`.* **Finding:** `native-image` **cannot** emit a static library (`.a`) — oracle/graal#3053 is still open on GraalVM 25 — and musl's run-anywhere property applies only to static *executables*, not shared libraries. So a fully-static, no-`.so` distribution that keeps the in-process FFI is not possible without re-architecting to a static musl executable behind IPC (rejected as too invasive). **Decision: pursue the real goal (distro/glibc independence) via a glibc-baseline build** — compile the Linux `.so` in `manylinux_2_28` (glibc 2.28) so it runs on RHEL/Ubuntu 20.04+/Debian 10+/AL2023. Experiment in `.github/workflows/static-linking-spike.yml`; fold into `ci.yml`/`release.yml` once green.
