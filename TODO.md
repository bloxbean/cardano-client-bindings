...
- [~] `P1` Publish wrappers to registries: PyPI (`ccl`), crates.io (`ccl`), npm (`@bloxbean/ccl`), and tag the Go module for the proxy. **Workflows prepared (gated):** `publish-python.yml` builds the 5 wheels (`auditwheel`→manylinux) and `publish-js.yml` builds the main + 5 per-platform `optionalDependencies` npm packages; both attach/publish on a version tag but keep the actual PyPI/npm upload behind an admin-set repo variable + Trusted-Publishing/token. `ccl` confirmed available on PyPI (checked 2026-07-01); an admin still needs to reserve it, register the trusted publisher, and flip the switch. crates.io + Go module tagging still to come.
