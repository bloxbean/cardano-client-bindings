...
- [~] `P0` Bundle or auto-fetch the native lib per wrapper so users no longer hand-set `CCL_LIB_PATH` / `DYLD_LIBRARY_PATH` / `LD_LIBRARY_PATH` — *decided; see [ADR-0012](docs/adr/0012-native-lib-bundled-in-wrapper-packages.md).* **Python: done** — `CclLib` now loads a `libccl.*` bundled inside the package (`ccl/_libs/`), falling back to `CCL_LIB_PATH` for local dev; `./gradlew :wrappers:python:wheel` builds a platform-tagged `py3-none-<platform>` wheel that ships the matching lib, so `pip install` needs no env vars (verified: install in a clean venv → `import ccl; CclLib()` works). **Remaining:** JS (npm `postinstall` fetch or bundled optionalDependencies), Rust (`build.rs` fetch or `include_bytes!`), Go (hardest — no install hooks; `go:embed` + extract-at-runtime, or document a fetch step); a CI job to build+publish the per-platform wheels/packages from the release artifacts (PyPI/npm/crates); and per-platform **wheels** for the now-built arches (native libs for `linux-aarch64` / `macos-x86_64` ship via the release matrix as of the arch-matrix PR; a musl variant is still to come).
